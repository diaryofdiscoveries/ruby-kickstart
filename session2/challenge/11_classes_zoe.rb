...
class BeerSong
  ...
  def print_stanza(n)
    if n.zero?
      ...
      puts "#{english_num n} #{bottle n} of beer on the wall,"        ,
           "#{english_num n} #{bottle n} of beer,"                    ,
           "Take one down, pass it around,"                         ,
           "#{english_num n - 1} #{bottle n-1} of beer on the wall."
    end
  end
  ...
  def english_num(n)
    if 0 <= n && n <= 19
      ...
      "#{english_num n/10*10}-#{english_num n%10}".downcase
    end.capitalize
  end
end

BeerSong.instance_methods false

beersong = BeerSong.new 99
puts beersong.print_song      
